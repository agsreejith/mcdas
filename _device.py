...
""" File:           maya.py
    Author:         Sree/iiaballoongroup 
		    (modified from Andreas Poehlmann code for USB2000) 
    Last change:    2014/09/04
...
class Maya200pro(object):
    ...
    def __init__(self):
        ...
        self._dev = usb.core.find(idVendor=0x2457, idProduct=0x102A)
        if self._dev is None:
            raise _OOError('No OceanOptics Maya2000pro spectrometer found!')
        else:
    ...
    def acquire_spectrum(self):
        raw_intensity = np.array(self._request_spectrum(), dtype=np.float)[10:]
        wavelength = sum( self._wl[i] * np.arange(10,2058)**i for i in range(4) )
        # fixed linearization, see documentation of mayapro 2000
